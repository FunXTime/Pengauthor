...
export async function searchPosts(
  ...
  const params = new URLSearchParams({search: query, per_page: 50, ...searchOptions });
  const results = await wpFetch(`wp/v2/search?${params}`, { site });
  let posts = await Promise.all(results
  ...
  if (filterTopTen) {
    ...
    posts = posts.filter(
      (post) => !post.categories?.includes(topTenCategory)
    );
  }
