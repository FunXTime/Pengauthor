...
import { normalizeHtml } from "./compare";
...
function parseParagraph(node) {
  ...
  const content = normalizeHtml(node.innerHTML.trim());
...
function parseBlockquote(node) {
  ...
  if (interview === "true") {
    ...
    return {
      _interview: true,
      _interviewType: "answer",
      content: answer?.outerHTML.trim() ?? normalizeHtml(node.innerHTML.trim())
    };
  }
  return {
    type: "blockquote",
...
function parseList(node) {
  const items = Array.from(node.children)
    ...
    .map((item) => normalizeHtml(item.innerHTML.trim()));
  return {
    type: "list",
    items,
    settings: {
